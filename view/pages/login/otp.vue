<template>
  <div class="flex justify-center items-center h-screen w-auto">
    ...
    <UForm :state="state" @submit="onSubmit" :validate="validate" @error="onError"
      ...
      <UButton type="submit" :label="label" :loading-icon="loadIcon" :loading="loading" class="flex justify-center items-center gap-1 py-2 rounded dark:text-custom-50" />
      <p class="text-red-500 dark:text-red-400 text-xs hidden" id="invalid-otp-message">Invalid OTP</p>
...
import { verifyOTP } from '~/assets/js/aws-sns';
...
async function onSubmit(event: FormSubmitEvent<any>) {
  ...
  setTimeout(() => {
    ...
    // verify the OTP
    const otpStatus = verifyOTP(state.otp);
    if (!otpStatus) {
      document.getElementById('invalid-otp-message').classList.remove('hidden');
    } else {
